...
command/verbatim/textblock methods only do push paragraphys 
into an array, and end_pod method do convertion.
...
specify using languals. 
not specifieds are ignore if it exists in mlpod document.
...
directories for existance check of file linked from pod.
by array reference.
